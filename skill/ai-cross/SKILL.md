...
**对开放式任务（架构取舍、方法学审查），"换厂商优于多想"已有直接证据**（植入缺陷审查实测，`bench/RESULTS-RIGOR-20260710.md` ⑤）：
- 硬伤扫查召回：关思考 **89%** vs 开思考 85%——**23× token 零回报**。硬伤多是单节内模式识别，按深度轴是**浅**操作；审查的"开放性"≠"深度"。
- 思考还可能**收紧判定阈值弄丢判断型缺陷**：LongCat 关思考全抓，开思考后 6/6 漏报全是"多重比较未校正/因果跳跃"——想了 3000 token 之后把它们排除出"硬伤"。
- **三家便宜模型关思考各扫一遍、取并集：召回 100%**（单模型最低 67%）。审查预算先买第二家厂商，再考虑更长思考。
- 未测的保留区：**跨节矛盾类深缺陷**（样本量与自由度对不上、跨节单位不一致）按深度轴预测该吃思考——拿不准时仍开。输出当 checklist 用，不当结论用。
...
**「架构/把关」给强模型 + 高 thinking，是因为它们需要比较多个假设、处理冲突约束**——不是因为"没有标准答案"（那条判据已作废）。⚠️ 但边际预算的顺序有实测依据：植入缺陷审查中，关思考召回 89% vs 开思考 85%（23× token 零回报），且思考可能收紧阈值弄丢判断型缺陷；**三家便宜模型关思考取并集召回 100%**。**故此处的边际预算优先给"换厂商拿独立第二意见"，而不是给"同一个模型想更久"；其输出当 checklist 用，不当结论用。** 例外保留：跨节矛盾类深缺陷按深度轴预测仍吃思考，拿不准时开。
