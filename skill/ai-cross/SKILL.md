---
name: ai-cross
description: 多模型分工与跨厂商交叉验证：把任务派给合适的模型、用不同厂商的模型互查关键产出、按档位分层保住订阅额度。支持 Claude subagent、codex CLI、GLM/Kimi coding plan、cc-switch 已配模型、裸 API/aichat。Use when 用户要求派发任务、分层执行、多模型分工、交叉验证、盘点/接入模型，或提到 dispatch、派工。
---
...
- **内部通道优先于外部通道**（实测，差一个数量级）：同一个 haiku 模型、同一个空任务——内部 subagent **3,963 token**，外部 `claude -p --model haiku` **30,230 token**，**差 7.6×**。**固定开销是「壳」的，不是「模型」的**：外派要新起一个 harness（系统提示 + 工具定义 + 技能索引），内部 subagent 共享宿主。
...
**判据是：从输入到答案，要走多少层相互依赖的变换？**

| 深度 | 特征 | thinking | 例子 |
|---|---|---|---|
| **浅** | 答案**已显式在输入里**，只需定位/复制/按固定规则转换 | **关** | 字段抽取、分类、格式转换、翻译、改写 |
| **中** | 需 ≥2 步相互依赖的推导（前一步错则全错） | **开，低-中** | 多步计算、条款交叉核对、常规调试 |
| **深** | 需比较多个假设、处理冲突约束、搜索多条路径 | **开，高** | 因果推断、架构取舍、大型代码库定位 |

**实测**（5 模型 × 10 任务 × n=5，裸 API，见 `bench/RESULTS-DEPTH-20260710.md`）——两组任务**都有唯一可验证答案**，唯一区别是答案在不在输入里：

| 深度 | 关思考 | 开思考 | 净效应 |
|---|---|---|---|
| 浅 | 99% | 100% | **+1pp，40× token** |
| 深 | **26%** | **91%** | **+65pp，165× token** |

**5/5 模型在深任务上均为正增益。** 所以：
- ⛔ **旧判据作废**：~~"只在没有机器可验证的答案时调高"~~ —— 数学证明、代码调试、约束求解都有唯一答案，却最吃推理。
- **深度轴是连续的**：抽取任务一旦加上"去重+排序"，就已有 +8pp 增益。

### ⛔ 铁律：「默认关思考」必须与「先判断深度」绑定，不得单独成立

深任务上关掉思考，模型**不会拒答**——实测 74 次失败**全部**是 `wrong`，零空输出、零拒答。它用 1–9 个 token、以完美格式给你一个错的数（11 步递推题关思考 **25/25 全错**，输出 `988`/`916`/`470`，正确答案 256）。
...
**对开放式任务（架构取舍、方法学审查）诚实声明**：没有可验证答案，长推理**可能**多想到一个约束，也**可能**只是产出一段更自洽的 rationalization，无反馈回路可区分。**这类任务上，换厂商拿独立第二意见比让同一个模型多想 2000 token 更可辩护；输出当 checklist 用，不当结论用。**
...
**但"档位不重要"≠"推理不重要"。** 同一批模型上，thinking 在深任务上是 **26% → 91%**。
...
**这一刀比档位更省钱。** 每次经 harness（`claude -p` / `codex exec` / subagent）派发都自带固定上下文足迹，其大头是**系统提示 + 工具定义**。实测同一句"你好"的 input：

| 通道 | prompt tokens | 相对裸 API |
|---|---|---|
| 裸 API（无系统提示） | **11** | 1× |
| 裸 API + 短系统提示 | 20 | 1.8× |
| 裸 API + ~500 字系统提示 | 241 | 22× |
| 内部 subagent（haiku） | 3,963 | 360× |
| codex exec | ~15,000 | ~1,360× |
| claude -p | ~30,000 | ~2,700× |

harness 税是裸调用的 **360×–2700×**。11→241 涨的是系统提示，241→30k 涨的是工具定义 + 技能索引。所以：
...
**thinking = 选搜索深度，按「答案在不在输入里」定**（见上方「推理强度 / thinking」节）。**不要用升档代替开思考**：档位几乎不影响正确率（+0pp），thinking 在深任务上是 +65pp。二者不可互相替代。
...
| 多步计算、条款交叉核对、常规调试 | 低/中档 | **中** | 执行（≥2 步相互依赖；**能写代码就让它写代码**） |
| 架构设计、任务拆解 | 中/高档 | **高** | 把关（多假设竞争、约束冲突） |
| 最终审查、统计/方法学把关 | 中/高档 | **高** | 把关（**换厂商** + 编排者独立核验） |
| 跨模型交叉审查 | 任意档 | 随任务深度 | 把关（**必须换厂商**） |
| 快速语法检查、格式化琐事 | 低档 | **关** | 侦察 |

**thinking 一列按「搜索深度」填，不是按「有没有标准答案」填。** 递推、约束求解、多跳算术全都有唯一可验证答案，关掉思考正确率 **26%**，开着 **91%**（实测，5/5 模型同向）。

**「架构/把关」给强模型 + 高 thinking，是因为它们需要比较多个假设、处理冲突约束**——不是因为"没有标准答案"（那条判据已作废）。⚠️ 但也要诚实：这类任务没有 verifier，长推理**可能**多想到一个约束，也**可能**只是产出一段更自洽的 rationalization。**故此处的边际预算优先给"换厂商拿独立第二意见"，而不是给"同一个模型想更久"；其输出当 checklist 用，不当结论用。**
...
- **升级阶梯（按实测收益排序，不是按传统 cascade）**：低档产出未过验证 / 出现 thrash 时，**依次**尝试：
  ...
  | ③ | **任务是深的却关着 thinking？→ 开 thinking** | **26% → 91%（+65pp，5/5 模型同向）** | 40–165× output token |
