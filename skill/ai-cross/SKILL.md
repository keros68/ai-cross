...
**在无内部通道的宿主（Codex/WorkBuddy/Qoder 等）**：路由表照用（它输出的是**档位**），只是低/中/高档由外部通道承担，`scout/worker/heavy` 仅作档位语义的占位名，不是可调用的角色。
...
推理强度管"想多久"。**默认低强度。** 实测：xhigh 相对 low 在可验证任务上**零准确率增益**，只多烧约 2× 推理 token（它们计入 output）。

**只在两种情况调高**：①任务**没有机器可验证的正确答案**（架构设计、方法学把关、论证审查）；②低强度产出已连续两次未过验证，且升级阶梯的前三步都试过了。

**不要错配**：mini + xhigh 通常不如换个厂商。各通道旋钮参数见 `references/channels.md`。

## 路由：三步走（档位是成本旋钮，不是能力旋钮）

四轮基准的结论：**「档位」几乎不影响正确率——唯一的例外，正是「模型亲自算」那类任务**。
...
**纯文本任务在支持的模型上关掉思考**（`enable_thinking=false` 或等价参数）。实测：纯文本任务不需要推理，推理模型开着思考纯烧 output token（output 是最贵的部分）——Qwen 抽取任务 512 → 18 token（**28×**），且从错变对。同一批纯文本任务，各模型 completion 中位从 40（DeepSeek）到 426（Qwen）差 **10×**，全是推理开销。**成本的真正变量是推理开销，不是模型档次。**
...
**已用美元核实**（Anthropic 官方单价，4 个硬任务合计）：haiku **$0.144**、sonnet $0.426、opus $0.404 —— **最低档便宜约 3 倍**。haiku 输出 token 是 opus 的 12.6 倍，但单价低 5 倍且 fresh 几乎全被缓存吸收，**话痨没吃掉价差**。
...
| 任务类型 | 默认 | 角色语义 |
|---|---|---|
| 扫描代码库/文档、定位、追调用链 | 低档 | 侦察 |
| 批量摘要、文献/数据初筛 | 低档 | 侦察（量大纯文本用 aichat 更省；**务必批量合并**） |
| 常规实现、修 bug、小重构、分析脚本 | 低档 | 执行 |
| 架构设计、任务拆解 | 中/高档 | 把关（判断类，非代码可验证，故仍给强模型） |
| 最终审查、统计/方法学把关 | 中/高档 | 把关（**换厂商** + 编排者独立核验） |
| 跨模型交叉审查 | 任意档 | 把关（**必须换厂商**） |
| 快速语法检查、格式化琐事 | 低档 | 侦察 |

「架构/把关」仍给强模型，是因为它们**没有机器可验证的正确答案**——不是因为强模型在可验证任务上更准（实测并不）。
...
- **升级阶梯（按实测收益排序，不是按传统 cascade）**：低档产出未过验证 / 出现 thrash 时，**依次**尝试：
  ...
  | ⑤ | **升推理强度 / 升档位**（**跳过中档，直接考虑高档**） | 内部三档仅在"亲自算"任务上有增益（haiku 67% → opus 100%） | 约 3× 成本 |

  **传统 cascade 把 ⑤ 当第一手，我们的数据说它是最后一手。** 前两步代价为零、收益最大，先做。
