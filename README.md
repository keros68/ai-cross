...
- **宿主 agent**：Claude Code 或 Codex 至少有一个。两者是一等支持目标；其中内部 subagent 分层（scout/worker/heavy/advisor）仅 Claude Code 有，Codex 宿主的低/中/高档全部由外部通道承担。
...
1. **盘点**（上面那句话）——生成 manifest，报告会告诉你：你的组合解锁了什么、缺什么、最便宜的补齐路径是什么。只有一家厂商时它会直说"交叉验证暂不可用"，不会假装能用。
2. **演示派发**（可选，推荐）——回一句"跑一次演示"，它会用一段内置的含缺陷代码（不碰你的项目文件）向两家厂商各发一次盲审，然后给你看三样东西：两家的共识、分歧、以及每路花了多少 token。这就是这个 skill 日常工作的样子。
3. **真实任务**——之后正常干活时带一句需求即可，例如：
...
- 外部派发必须留痕，关键任务要能回看原始输出；`.dispatch/` 留痕目录默认写入 `.gitignore`，任务原文与模型输出不随代码库提交。
...
- `references/evidence.md` - 各规则背后的实测证据汇编（样本量与局限如实标注）。
